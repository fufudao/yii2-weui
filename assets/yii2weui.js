...
/***/ function(module, exports, __webpack_require__) {
	...
	/* actionSheet */
	if(document.querySelector('#actionSheetBtn') !== null){
		document.querySelector('#actionSheetBtn').addEventListener('click', function () {
		    _weui2.default.actionSheet([{
		        ...
		        label: '发布服务',
		        onClick: function onClick() {
		            window.location.href="/bwechat/service/create";  
		        }
		    }], {
	...
	if(document.querySelector('#logout_btn') !== null){
		document.querySelector('#logout_btn').addEventListener('click', function () {
		    _weui2.default.confirm('确定退出吗？', function () {
		        // console.log('yes');
		        window.location.href="/bwechat/site/logout"; 
		    }, function () {
		        // console.log('no');
		    }, {
		        title: '确定退出吗？'
	...
	/* 用户管理 删除提示 confirm */
	if(document.querySelector('#tableBox') !== null){
		var aTag = document.querySelector('#tableBox').getElementsByTagName('a');
		if(aTag.length > 0){
			for (var i = 0; i < aTag.length; i++) {
				if(aTag[i].getAttribute('link')!==null){
					aTag[i].addEventListener('click', function () {
			        	var link = this.getAttribute('link');
		            	_weui2.default.confirm('确定删除吗？', function () {
					        // console.log('yes');
					        // console.log(link);
	...
	/* 图片自动上传 */
	if(document.querySelector('#uploader') !== null){

		var uploadCount = parseInt(document.getElementById("uploadCount").innerHTML),
			uploadMaxCount = parseInt(document.getElementById("uploadMaxCount").innerHTML),
		    uploadList = [];

		    // console.log(++uploadCount);

		var uploadCountDom = document.getElementById("uploadCount");
		var uploaderInputDom = document.getElementById("uploaderInput");
		_weui2.default.uploader('#uploader', {
		    url: 'http://' + location.hostname + ':80/upload-file/upload',
		    auto: true,
		    type: 'file',
		    fileVal: 'fileVal',
		    compress: {
		    ...
		    onBeforeQueued: function onBeforeQueued(files) {
		        ...
		        if (files.length > uploadMaxCount) {
		            // 防止一下子选中过多文件
		            _weui2.default.alert('最多只能上传'+uploadMaxCount+'张图片，请重新选择');
		            return false;
		        }
		        if (uploadCount + 1 > uploadMaxCount) {
		            _weui2.default.alert('最多只能上传'+uploadMaxCount+'张图片');
		            return false;
		        }
		    ...
		    onSuccess: function onSuccess(ret) {
		    	//上传成功后，把图片id放入隐藏域
		        if(ret.responseText.length == 13){
		        	var forId =	uploaderInputDom.getAttribute('for');
		        	if(forId.length > 0){
	        			var hideInput = document.getElementById(forId);
		        		if(hideInput.getAttribute('value')==null || hideInput.getAttribute('value')=='')
		        		{
		        			hideInput.setAttribute('value', ret.responseText);
		        		}else{
		        			var valueStr = hideInput.getAttribute('value')+','+ret.responseText;
		        			hideInput.setAttribute('value', valueStr);
		        		}
		        		//保存当前模型图片字段的值。
     					//暂时模拟提交当前form用来保存图片
     					document.querySelector("#formSubmitBtn").click();
		        	}
		        }else{
		        	console.log('上传失败');
		        }
		    ...
		    var gallery = _weui2.default.gallery(url, {
		        ...
		        onDelete: function onDelete() {
		            _weui2.default.confirm('确定删除该图片？', function () {
		            	...
		            	if(forId.length > 0){
		        			var hideInput = document.getElementById(forId);
			        		if(hideInput.getAttribute('value').length > 0)
			        		{
			        			var valueStr = hideInput.getAttribute('value');
			        			var valueArr = valueStr.split(',');
			        			var key = id-1;
			        			//被删除的图片id
			        			var delImgId = valueArr[key];

			        			//后台删除对应的图片
				                //新建异步处理对象
			             		var ajax = new XMLHttpRequest();
			                    //设置请求类型为post,请求的页面为server1.php
			             		ajax.open('post','/upload-file/delete-upload');
			         			//因为是post请求所以必须加上setRequestHeader(get请求可以忽略)
			         			ajax.setRequestHeader('Content-type','application/x-www-form-urlencoded');
			                	//请求所带的参数 name=this.value
			         			ajax.send('delete_upload_id='+delImgId);
			             			...
			             			if (ajax.readyState==4&&ajax.status==200) {
				                		...
				                		gallery.hide();
			             				if(ajax.responseText == true){
			             					...
			             					//后台数据删除成功后
			             					//修改隐藏域的值
			             					valueArr.splice(key, 1);
			        						var newValue = valueArr.join(',');
			        						hideInput.setAttribute('value', newValue);
	...
	//服务添加
	if(document.querySelector('#cascadePickerBtn2') !== null){
		/* picker */
		// 级联选择器
	document.querySelector('#cascadePickerBtn2').addEventListener('click', function () {
		//新建异步处理对象
 		var ajax = new XMLHttpRequest();
 		ajax.open('get','get-service-classify');
			//因为是post请求所以必须加上setRequestHeader(get请求可以忽略)
			ajax.setRequestHeader('Content-type','application/x-www-form-urlencoded');
			ajax.send();
    	//绑定onreadystatechange事件
 				...
 				if(ajax.responseText !== false){
 					...
 					_weui2.default.picker(data, {
				        ...
				        onConfirm: function onConfirm(result) {
				        	//服务添加时的js
				        	if(document.getElementById('SService[type]') !== null){
				        		document.getElementById("SService[type]").value=result[result.length-1].value;
				        		document.getElementById("cascadePickerBtn2").getElementsByTagName('option')[0].innerHTML=result[result.length-1].label;
				        		return false;
				        	}
				        	//服务修改时的js
				        	var categoryId=result[result.length-1].value;
				        	var categoryId2=document.getElementById('cascadePickerBtn2').getAttribute('type');
				        	if(categoryId == categoryId2){
				        		_weui.toast('操作成功', 1000);
				        		return false;
				        	}
				            var ajax = new XMLHttpRequest();
				            ajax.open('post','/bwechat/service/update-service');
				            ajax.setRequestHeader('Content-type','application/x-www-form-urlencoded');
				            var id=document.getElementById("serviceId").value;
				            var enableCsrfValidation=document.getElementById("enableCsrfValidation").value;
				            ajax.send('serviceId='+id+'&colume=type&value='+result[result.length-1].value+'&_csrf='+enableCsrfValidation);
				            ajax.onreadystatechange=function(){
				            	if (ajax.readyState==4&&ajax.status==200) {
				            		if(ajax.responseText === '1'){
				            			document.getElementById("cascadePickerBtn2").getElementsByTagName("p")[2].innerHTML = result[result.length-1].label;
				            			document.getElementById('cascadePickerBtn2').setAttribute('type',result[result.length-1].value);
				            			console.log('修改成功');
				            			_weui.toast('操作成功', 1000);
				            		}else{
				            			console.log('修改失败');
	...
	//服务页面添加关联产品
	if(document.querySelector('#cascadePickerBtn3') !== null){
		/* picker */
		// 级联选择器
	//ssGoodIds的缓存
	document.querySelector('#cascadePickerBtn3').addEventListener('click', function () {
        //设置请求类型为get,请求的页面为server1.php
        var name=document.getElementById("searchInput").value;
			var ajax = new XMLHttpRequest();
 			ajax.open('get','/bwechat/service/get-goods?name='+name);
			//因为是post请求所以必须加上setRequestHeader(get请求可以忽略)
			ajax.setRequestHeader('Content-type','application/x-www-form-urlencoded');
    		//请求所带的参数 name=this.value
			ajax.send();
					...
					if(ajax.responseText !== false){
						...
						_weui2.default.picker(data, {
						        ...
						        onConfirm: function onConfirm(result) {
						        	...
						        	var label=result[result.length-1].label;
						        	var value=result[result.length-1].value;
						        	...
						        	var ajax = new XMLHttpRequest();
						            ajax.open('get','/bwechat/service/get-img-by-sgoodid?s_good_id='+value);
						            ajax.setRequestHeader('Content-type','application/x-www-form-urlencoded');
						            ajax.send();
						            ajax.onreadystatechange=function(){
						            	if (ajax.readyState==4&&ajax.status==200) {
						            			...
						            			// console.log(data);
						            			var str='';
												str+='<div class="weui-cell" style="border-bottom:1px solid #d9d9d9;">';
								                str+='    <div class="weui-cell__hd" style="position: relative;margin-right: 10px;">';
								                if(data !== false){
								                	str+='        <img src="/'+data+'" alt="" style="width: 70px;display: block;">';
								                }
								                str+='    </div>';
								                str+='    <div class="weui-cell__bd"><p>'+label+'</p></div>';
								                str+='    <div class="weui-cell__ft"><p><i class="weui-icon-cancel relatedGoodsDelete" onClick="window.deleteGood()"></i></p></div>';
								                str+='<input name=relatedGoods[] type=hidden value='+value+'>';
								                str+='</div>';
								                document.getElementById("relatedGoods").insertAdjacentHTML("beforeend",str);
						            	}
						            }
						        },
						        id: 'cascadePicker'
	...
	//产品添加
	if(document.querySelector('#cascadePickerBtn') !== null){
		/* picker */
		// 级联选择器
	document.querySelector('#cascadePickerBtn').addEventListener('click', function () {
		//新建异步处理对象
 		var ajax = new XMLHttpRequest();
        //设置请求类型为post,请求的页面为server1.php
 		ajax.open('post','/upload-file/get-classify');
			//因为是post请求所以必须加上setRequestHeader(get请求可以忽略)
			ajax.setRequestHeader('Content-type','application/x-www-form-urlencoded');
    	//请求所带的参数 name=this.value
			ajax.send();
 			...
 			if (ajax.readyState==4&&ajax.status==200) {
 				if(ajax.responseText !== false){
 					...
 					_weui2.default.picker(data, {
				        ...
				        onConfirm: function onConfirm(result) {
				        	//产品添加时的js
				        	if(document.getElementById('BSysGoods[s_good_category_id]') !== null){
				        		document.getElementById("BSysGoods[s_good_category_id]").value=result[result.length-1].value;
				        		document.getElementById("cascadePickerBtn").getElementsByTagName('option')[0].innerHTML=result[result.length-1].label;
				        		return false;
				        	}
				        	//产品修改时的js
				        	var categoryId=result[result.length-1].value;
				        	var categoryId2=document.getElementById('cascadePickerBtn').getAttribute('categoryid');
				        	if(categoryId === categoryId2){
				        		// console.log('=');
				        		_weui.toast('操作成功', 1000);
				        		return false;
				            ...
				            var ajax = new XMLHttpRequest();
				            // ajax.open('post','/upload-file/update-good');
				            ajax.open('post','/bwechat/sys-goods/update-good');
				            ajax.setRequestHeader('Content-type','application/x-www-form-urlencoded');
				            var id=document.getElementById("sysGoodId").value;
				            var enableCsrfValidation=document.getElementById("enableCsrfValidation").value;
				            ajax.send('sysgoodId='+id+'&colume=s_good_category_id&value='+result[result.length-1].value+'&_csrf='+enableCsrfValidation);
				            ajax.onreadystatechange=function(){
				            	if (ajax.readyState==4&&ajax.status==200) {
				            		if(ajax.responseText === '1'){
				            			document.getElementById("cascadePickerBtn").getElementsByTagName("p")[2].innerHTML = result[result.length-1].label;
				            			document.getElementById('cascadePickerBtn').setAttribute('categoryid',result[result.length-1].value);
				            			console.log('修改成功');
				            			_weui.toast('操作成功', 1000);
				            		}else{
				            			console.log('修改失败');
...
if(document.querySelector('#pickerBtn') !== null){
	// 普通选择器
	document.querySelector('#pickerBtn').addEventListener('click', function () {
	    _weui2.default.picker([{
	        ...
	        onConfirm: function onConfirm(result) {
	            document.getElementById("pickerBtn").getElementsByTagName("p")[2].innerHTML = result[0]['label'];
	                //新建异步处理对象
	                var tuijian=document.getElementById('pickerBtn').getAttribute('tuijian');
		        	if(result[result.length-1].label === tuijian){
		        		_weui.toast('操作成功', 1000);
		        		return false;
		        	}
             		var ajax = new XMLHttpRequest();
             		if(document.getElementById('serviceId') !== null){
             			var id=document.getElementById("serviceId").value;
             			var url='/bwechat/service/update-service';
             			var enableCsrfValidation=document.getElementById("enableCsrfValidation").value;
             			var send='serviceId='+id+'&colume=isShowcase&value='+result[result.length-1].value+'&_csrf='+enableCsrfValidation;
             		}else if(document.getElementById('sysGoodId') !== null){
             			var id=document.getElementById("sysGoodId").value;
             			var url='/bwechat/sys-goods/update-good';
             			var enableCsrfValidation=document.getElementById("enableCsrfValidation").value;
             			var send='sysgoodId='+id+'&colume=isShowcase&value='+result[0]['value']+'&_csrf='+enableCsrfValidation;
             		}else{
             			_weui.toast('页面错误', 1000);
             		}
                    //设置请求类型为post,请求的页面为server1.php
             		ajax.open('post',url);
         			//因为是post请求所以必须加上setRequestHeader(get请求可以忽略)
                		...
                		//判断如果请求的状态是否成功
             			if (ajax.readyState==4&&ajax.status==200) {
             				...
             				if(ajax.responseText == true){
             					console.log('修改成功');
             					// showTips('修改成功');
             					document.getElementById('pickerBtn').setAttribute('tuijian',result[result.length-1].label)
             					_weui.toast('操作成功', 1000);
             					//后台数据删除成功后
        						//保存当前模型图片字段的值。
             					//暂时模拟提交当前form用来保存
...
//弹框选择性别（在c端修改个人信息时调用）
if(document.querySelector('#pickerBtn2') !== null){
	// 普通选择器
	document.querySelector('#pickerBtn2').addEventListener('click', function () {
	    _weui2.default.picker([{
	        label: '男',
	        value: 'man'
	    },{
	        ...
	        onConfirm: function onConfirm(result) {
	            document.getElementById("pickerBtn2").getElementsByTagName("p")[2].innerHTML = result[0]['label'];
	                //新建异步处理对象
	                var sex=document.getElementById('pickerBtn2').getAttribute('sex');
		        	if(result[result.length-1].label === sex){
		        		_weui.toast('操作成功', 1000);
		        		return false;
		        	}
             		var ajax = new XMLHttpRequest();
             		if(document.getElementById('accountId') !== null){
             			...
             			var url='/cwechat/user-center/update-gender';
             			var enableCsrfValidation=document.getElementById("enableCsrfValidation").value;
             			var send='accountId='+id+'&colume=gender&value='+result[result.length-1].value+'&_csrf='+enableCsrfValidation;
             		}else{
             			_weui.toast('页面错误', 1000);
             		}
                    //设置请求类型为post,请求的页面为server1.php
                		...
                		//判断如果请求的状态是否成功
             			if (ajax.readyState==4&&ajax.status==200) {
             				...
             				if(ajax.responseText == true){
             					console.log('修改成功');
             					// showTips('修改成功');
             					document.getElementById('pickerBtn2').setAttribute('sex',result[result.length-1].label)
             					_weui.toast('操作成功', 1000);
             					//后台数据删除成功后
        						//保存当前模型图片字段的值。
             					//暂时模拟提交当前form用来保存
	...
	/* 图片自动上传 */
	if(document.querySelector('#uploader2') !== null){
		...
		var uploadCountDom = document.getElementById("uploadCount");
		var uploaderInputDom = document.getElementById("uploaderInput");
		_weui2.default.uploader('#uploader2', {
		    url: 'http://' + location.hostname + ':80/upload-file/upload',
		    auto: true,
		    type: 'file',
		    fileVal: 'fileVal',
		    ...
		    onBeforeQueued: function onBeforeQueued(files) {
		        ...
		        if (document.getElementById("uploaderFiles").getElementsByTagName('li').length+1 > uploadMaxCount) {
		            _weui2.default.alert('最多只能上传'+uploadMaxCount+'张图片');
		            return false;
		        }
		    },
		    ...
		    onSuccess: function onSuccess(ret) {
		        ...
		        if(ret.responseText.length == 13){
		        	...
		        	if(forId.length > 0){
		        		...
		        		var liLength = document.getElementById("uploaderFiles").getElementsByTagName('li').length;
		        		uploadCountDom.innerHTML = liLength;
		        	}
		        }else{
		        	console.log('上传失败');
		        }
			             			...
			             			if (ajax.readyState==4&&ajax.status==200) {
				                		...
				                		gallery.hide();
			             				if(ajax.responseText == true){
			        						...
			        						var newValue = valueArr.join(',');
			        						hideInput.setAttribute('value', newValue);
			        						document.getElementById("uploadCount").innerHTML = document.getElementById("uploadCount").innerHTML-1;
			        						//保存当前模型图片字段的值。
			             					//暂时模拟提交当前form用来保存
			             				}else{
			             					console.log('删除失败');
		...
		/* c端用户图片自动上传 */
	if(document.querySelector('#uploader3') !== null){
		...
		var uploadCountDom = document.getElementById("uploadCount");
		var uploaderInputDom = document.getElementById("uploaderInput");
		_weui2.default.uploader('#uploader3', {
		    url: 'http://' + location.hostname + ':80/upload-file/upload2',
		    auto: true,
		    type: 'file',
		    fileVal: 'fileVal',
		    compress: {
		            	...
		            	if(forId.length > 0){
			        		...
			        		{	
				                ...
				                //新建异步处理对象
			             		var ajax = new XMLHttpRequest();
			                    //设置请求类型为post,请求的页面为server1.php
			             		ajax.open('post','/upload-file/delete-upload2');
			         			//因为是post请求所以必须加上setRequestHeader(get请求可以忽略)
			         			ajax.setRequestHeader('Content-type','application/x-www-form-urlencoded');
			                	//请求所带的参数 name=this.value
			         			ajax.send('delete_upload_id='+delImgId);
...
/***/ function(module, exports, __webpack_require__) {
	...
	function _validate($input, $form, regexp) {
	    ...
	    if (input.tagName == 'INPUT' || input.tagName == 'TEXTAREA') {
	        //var reg = input.getAttribute('pattern') || ''; tyt
	        var reg = input.getAttribute('wxpattern') || '';
...
/***/ function(module, exports, __webpack_require__) {
	...
	// var _id = 0;  //weui原版
	//zxb
	if(typeof(uploadCount)=="undefined"){var _id=0;}else{var _id=uploadCount.innerHTML;}
